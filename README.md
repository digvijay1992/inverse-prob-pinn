...
The physical setup is identical to your forward PINN case:[file:11]
...
with \(a_n = 8/(\pi^3 n^3)\) for odd \(n\) and zero for even \(n\), evaluated up to \(N = 200\) modes.
...
- Sensor data loss:
  ...
  - `data_loss(phi, θ, p)` compares network predictions at `sensor_input` to `sensor_target`, normalized by the number of measurements.[file:11]
...
Multiple static figures and animations are generated for analysis and presentations:[file:11][file:10][file:12][file:13][file:14]
...
- Static PNGs:
  - `inverse_c_wave_pinn_comparison.png`: 2×2 panel showing:
    ...
    - Heatmap of true solution.\[file:11][file:13]
...
| Fourier modes | `Nfourier` | 200 | Modes in analytical series. |
